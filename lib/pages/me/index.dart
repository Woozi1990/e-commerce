...
class _MeViewState extends State<MeView> {
  ...
  bool _hasMore = true;
  Future<void> _getMeRecommendedList() async {
    if (_isLoading || !_hasMore) return;
    _isLoading = true;
    final result = await getMeRecommendListAPI(_meRecommendedParams);
    _isLoading = false;
    setState(() {});
    _meRecommendedList.addAll(result.items);
    if (_meRecommendedParams["page"] >= result.pages) {
      _hasMore = false;
      return;
    }
    _meRecommendedParams['page'] += 1;
  }
  ...
  Widget build(BuildContext context) {
    return CustomScrollView(
      ...
      slivers: [
        ...
        SliverPersistentHeader(pinned: true, delegate: MeRecommendedSection()),
        ProductFeedSection(productFeedList: _meRecommendedList),
      ],
    );
  }
}
